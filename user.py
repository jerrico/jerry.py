...
class Restriction(object):
    ...
    def allows(self, attr, *args, **kwargs):
        return False

    def did(self, attr, *args, **kwargs):
        pass
...
class JerryUser(object):
    ...
    def can(self, action, *args, **kwargs):
        ...
        for restriction in restrictions:
            if not restriction.allows(action, *args, **kwargs):
                return False

        return not self.default

    def did(self, action, *args, **kwargs):
        restrictions = self.restrictions.get(action, "")
        if restrictions:
            for restriction in restrictions:
                restriction.did(action, *args, **kwargs)
...
if __name__ == "__main__":
    ...
    user.can("upload_photo", 4).should.be.false

    user.can("share_photo").should.be.ok
    user.can("share_photo", 9).should.be.ok
    # total amount kicks in though daily woud allow it
    user.can("share_photo", 12).should.be.false

    # not defined: default is deny
    user.can("view_photo").should.be.false
    user.can("view_photo", 10).should.be.false
    ...
    user.can("upload_photo").should.be.false
